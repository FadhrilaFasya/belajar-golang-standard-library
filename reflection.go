...
// readField membaca tag pada struct
func readField(value any) {
	var valueType reflect.Type = reflect.TypeOf(value)
	fmt.Println("Type Name", valueType.Name())

	// membaca field pada struct
...
// IsValid mengecek apakah field yang required sudah diisi
func IsValid(value any) (result bool) {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if f.Tag.Get("required") == "true" {
			data := reflect.ValueOf(value).Field(i).Interface() // ambil data field
			// cek data field
			result = data != ""
			if result == false {
				return result
